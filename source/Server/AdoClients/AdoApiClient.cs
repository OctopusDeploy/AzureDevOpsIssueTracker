...
{
    internal interface IAdoApiClient
    {
    ...
    internal class AdoApiClient : IAdoApiClient
    {
            ...
            bool testing = false)
        {
            ...
            if (status.HttpStatusCode == HttpStatusCode.NotFound) return ResultFromExtension<(int id, string url)[]>.Success(new (int, string)[0]);

            if (!status.IsSuccessStatusCode())
                return ResultFromExtension<(int id, string url)[]>.Failed(
                    $"Error while fetching work item references from Azure DevOps: {status.ToDescription(jObject, testing)}");
            ...
            string? personalAccessToken = null, bool testing = false)
        {
            ...
            if (!status.IsSuccessStatusCode())
                return ResultFromExtension<(string title, int? commentCount)>.Failed(
                    $"Error while fetching work item details from Azure DevOps: {status.ToDescription(jObject, testing)}");
            ...
            try
            {
                ...
                if (fields == null)
                    return ResultFromExtension<(string title, int? commentCount)>.Failed(
                        "Unable to interpret work item details from Azure DevOps. `fields` element is missing.");

                return ResultFromExtension<(string title, int? commentCount)>.Success((fields?["System.Title"]?.ToString() ?? string.Empty,
                    fields?["System.CommentCount"]?.Value<int>() ?? default(int)));
            }
            catch
            {
        ...
        public IResultFromExtension<string[]> GetWorkItemComments(AdoProjectUrls adoProjectUrls, int workItemId)
        {
            ...
            if (status.HttpStatusCode == HttpStatusCode.NotFound) return ResultFromExtension<string[]>.Success(new string[0]);
        ...
        private string BuildWorkItemBrowserUrl(AdoProjectUrls adoProjectUrls, int workItemId)
        {
            // ReSharper disable once StringLiteralTypo
            return $"{adoProjectUrls.ProjectUrl}/_workitems?_a=edit&id={workItemId}";
        }

        private string? GetReleaseNote(AdoProjectUrls adoProjectUrls, int workItemId, int? commentCount = null)
        {
            var releaseNotePrefix = store.GetReleaseNotePrefix();
            if (string.IsNullOrWhiteSpace(releaseNotePrefix) || commentCount == 0) return null;
        ...
        private IResultFromExtension<WorkItemLink> GetWorkItemLink(AdoProjectUrls adoProjectUrls, int workItemId)
        {
